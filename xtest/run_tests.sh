...
for F in "$@"; do
  nasm-0.98.39 -DCMDVAL=15 -O9 -f bin -o "$F".nasm98o9.bin "$F".nasm
  nasm-0.98.39 -DCMDVAL=15 -DO01=1 -DO0=1 -O0 -f bin -o "$F".nasm98.bin "$F".nasm
  ndisasm -b 16 -o 0x100 "$F".nasm98.bin >"$F".nasm98.ndisasm
  ndisasm -b 16 -o 0x100 "$F".nasm98o9.bin >"$F".nasm98o9.ndisasm
  ../mininasm -DCMDVAL=15 -DO01=1 -DO0=1 -f bin -o "$F".mininasm.bin "$F".nasm
  ../mininasm -DCMDVAL=15 -DO01=1 -O1 -f bin -o "$F".mininasmo1.bin "$F".nasm  # Not compared against anything.
  ../mininasm -DCMDVAL=15 -O9 -f bin -o "$F".mininasmo9.bin "$F".nasm
